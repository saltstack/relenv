...
def test_sbom_copy_python_312(tmp_path: pathlib.Path) -> None:
    """Test that SBOM files are copied from Python 3.12+ source to prefix directory."""
    import json

    from relenv.build.common import Dirs, copy_sbom_files
    from relenv.common import WorkDirs

    # Create mock directory structure
    root = tmp_path / "relenv_root"
    src_dir = root / "src"
    build_dir = root / "build"

    # Create Python source directory with SBOM files
    python_src = src_dir / "Python-3.12.8"
    misc_dir = python_src / "Misc"
    ...
    externals_content = {
        ...
        "dataLicense": "CC0-1.0",
        "SPDXID": "SPDXRef-DOCUMENT",
        "name": "CPython-3.12.8-externals",
    }

    (misc_dir / "sbom.spdx.json").write_text(json.dumps(sbom_content))
    (misc_dir / "externals.spdx.json").write_text(json.dumps(externals_content))

    # Create WorkDirs and Dirs objects
    work_dirs = WorkDirs(root=root)
    # Override the default paths to use our mock structure
    work_dirs.build = build_dir
    work_dirs.src = src_dir
    work_dirs.logs = root / "logs"

    dirs = Dirs(work_dirs, "test", "x86_64", "3.12.8")

    # Create prefix directory with minimal required structure
    # (dirs.prefix is computed from dirs.build / version-triplet)
    prefix_dir = dirs.prefix
    ...
    # Call copy_sbom_files (this should copy SBOM files)
    copy_sbom_files(dirs)

    # Verify SBOM files were copied to prefix directory
    dest_sbom = prefix_dir / "sbom.spdx.json"
    dest_externals = prefix_dir / "externals.spdx.json"

    assert dest_sbom.exists(), "sbom.spdx.json should be copied to prefix directory"
    assert (
        dest_externals.exists()
    ), "externals.spdx.json should be copied to prefix directory"

    # Verify content is correct
    assert json.loads(dest_sbom.read_text())["name"] == "CPython-3.12.8"
    assert json.loads(dest_externals.read_text())["name"] == "CPython-3.12.8-externals"


def test_sbom_copy_python_310(tmp_path: pathlib.Path) -> None:
    """Test that SBOM files are gracefully skipped for Python < 3.12."""
    from relenv.build.common import Dirs, copy_sbom_files
    from relenv.common import WorkDirs
    ...
    # Create Python source directory WITHOUT SBOM files (Python 3.10)
    python_src = src_dir / "Python-3.10.18"
    misc_dir = python_src / "Misc"
    misc_dir.mkdir(parents=True)

    # Create some other files but NOT SBOM files
    (misc_dir / "README").write_text("Python 3.10 Misc directory")
    ...
    dirs = Dirs(work_dirs, "test", "x86_64", "3.10.18")
    ...
    # Call copy_sbom_files (this should gracefully skip SBOM files)
    copy_sbom_files(dirs)

    # Verify SBOM files were NOT copied (they don't exist in source)
    dest_sbom = prefix_dir / "sbom.spdx.json"
    dest_externals = prefix_dir / "externals.spdx.json"

    assert not dest_sbom.exists(), "sbom.spdx.json should not exist for Python < 3.12"
    assert (
        not dest_externals.exists()
